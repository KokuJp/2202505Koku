...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        $sql = "INSERT INTO users (username, password,fullname,hobby) VALUES (?, ?, ?, ?)";
         //Tham số trong bảng có thể thay đổi thứ tự, tuy nhiên dữ liệu tương ứng.
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssss", $param_username, $param_password,$fullname,$hobby);
...
<body>
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <p>すでにアカウント餅の方? <a href="login.php">ログイン</a>.</p>
        </form>
    </div>    
</body>
</html>